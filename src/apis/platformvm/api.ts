...
import { Defaults, PlatformChainID, ONECFLT } from "../../utils/constants"
...
export class PlatformVMAPI extends JRPCAPI {
  ...
  protected CFLTAssetID: Buffer = undefined
  ...
  /**
   * Fetches the CFLT AssetID and returns it in a Promise.
   *
   * @param refresh This function caches the response. Refresh = true will bust the cache.
   *
   * @returns The the provided string representing the CFLT AssetID
   */
  getCFLTAssetID = async (refresh: boolean = false): Promise<Buffer> => {
    if (typeof this.CFLTAssetID === "undefined" || refresh) {
      const assetID: string = await this.getStakingAssetID()
      this.CFLTAssetID = bintools.cb58Decode(assetID)
    }
    return this.CFLTAssetID
  }

  /**
   * Overrides the defaults and sets the cache to a specific CFLT AssetID
   *
   * @param cfltAssetID A cb58 string or Buffer representing the CFLT AssetID
   *
   * @returns The the provided string representing the CFLT AssetID
   */
  setCFLTAssetID = (cfltAssetID: string | Buffer) => {
    if (typeof cfltAssetID === "string") {
      cfltAssetID = bintools.cb58Decode(cfltAssetID)
    }
    this.CFLTAssetID = cfltAssetID
  }
  ...
  checkGooseEgg = async (
    ...
    const cfltAssetID: Buffer = await this.getCFLTAssetID()
    let outputTotal: BN = outTotal.gt(new BN(0))
      ? outTotal
      : utx.getOutputTotal(cfltAssetID)
    const fee: BN = utx.getBurn(cfltAssetID)
    if (fee.lte(ONECFLT.mul(new BN(10))) || fee.lte(outputTotal)) {
      return true
    } else {
      return false
    }
   ...
   *
   * @returns Promise for an array of validators that are currently staking, see: {@link https://docs.coinflect.com/v1.0/en/api/platform/#platformgetcurrentvalidators|platform.getCurrentValidators documentation}.
   *
   */
   ...
   * @param nodeIDs Optional. An array of strings
   *
   * @returns Promise for an array of validators that are pending staking, see: {@link https://docs.coinflect.com/v1.0/en/api/platform/#platformgetpendingvalidators|platform.getPendingValidators documentation}.
   *
   */
   ...
   * @param stakeAmount The amount of nCFLT the validator is staking as
   ...
   * @param stakeAmount The amount of nCFLT the delegator is staking as
   * a {@link https://github.com/indutny/bn.js/|BN}
   * @param rewardAddress The address of the account the staked CFLT and validation reward
   ...
   *
   * @param subnetID Either a {@link https://github.com/feross/buffer|Buffer} or an CFLT
  ...
  /**
   * Send CFLT from an account on the P-Chain to an address on the X-Chain. This transaction
   * must be signed with the key of the account that the CFLT is sent from and which pays the
   * transaction fee. After issuing this transaction, you must call the X-Chain’s importCFLT
   * method to complete the transfer.
   *
   * @param username The Keystore user that controls the account specified in `to`
   * @param password The password of the Keystore user
   * @param to The address on the X-Chain to send the CFLT to. Do not include X- in the address
   * @param amount Amount of CFLT to export as a {@link https://github.com/indutny/bn.js/|BN}
   *
   * @returns Promise for an unsigned transaction to be signed by the account the the CFLT is
  ...
  /**
   * Send CFLT from an account on the P-Chain to an address on the X-Chain. This transaction
   * must be signed with the key of the account that the CFLT is sent from and which pays
   * the transaction fee. After issuing this transaction, you must call the X-Chain’s
   * importCFLT method to complete the transfer.
   *
   * @param username The Keystore user that controls the account specified in `to`
   * @param password The password of the Keystore user
   * @param to The ID of the account the CFLT is sent to. This must be the same as the to
   * argument in the corresponding call to the X-Chain’s exportCFLT
  ...
  /**
   * getMaxStakeAmount() returns the maximum amount of nCFLT staking to the named node during the time period.
   *
   ...
   *
   * @param utxoset A set of UTXOs that the transaction is built on.
   * @param fromAddresses An array of addresses as {@link https://github.com/feross/buffer|Buffer} who pays the fees in CFLT
   * @param changeAddresses An array of addresses as {@link https://github.com/feross/buffer|Buffer} who gets the change leftover from the fee payment
   * @param nodeID The node ID of the validator being added.
   * @param startTime The Unix time when the validator starts validating the Primary Network.
   * @param endTime The Unix time when the validator stops validating the Primary Network (and staked CFLT is returned).
   ...
   * @param fromAddresses An array of addresses as {@link https://github.com/feross/buffer|Buffer} who own the staking UTXOs the fees in CFLT
   * @param changeAddresses An array of addresses as {@link https://github.com/feross/buffer|Buffer} who gets the change leftover from the fee payment
   * @param nodeID The node ID of the validator being added.
   * @param startTime The Unix time when the validator starts validating the Primary Network.
   * @param endTime The Unix time when the validator stops validating the Primary Network (and staked CFLT is returned).
   ...
   * Instead use the [[Coinflect.addAPI]] method.
   *
   ...
   */
  constructor(core: CoinflectCore, baseURL: string = "/ext/bc/P") {
    super(core, baseURL)
