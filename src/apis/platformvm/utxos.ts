...
export class UTXOSet extends StandardUTXOSet<UTXO> {
   ...
   * @param cfltAssetID {@link https://github.com/feross/buffer|Buffer} of the asset ID for CFLT
   * @param toAddresses An array of addresses as {@link https://github.com/feross/buffer|Buffer} who recieves the CFLT
   * @param fromAddresses An array of addresses as {@link https://github.com/feross/buffer|Buffer} who owns the CFLT
   * @param changeAddresses An array of addresses as {@link https://github.com/feross/buffer|Buffer} who gets the change leftover of the CFLT
  ...
  buildExportTx = (
    ...
    blockchainID: Buffer,
    amount: BN,
    cfltAssetID: Buffer, // TODO: rename this to amountAssetID
    toAddresses: Buffer[],
    fromAddresses: Buffer[],
    changeAddresses: Buffer[] = undefined,
    ...
    if (typeof feeAssetID === "undefined") {
      feeAssetID = cfltAssetID
    } else if (feeAssetID.toString("hex") !== cfltAssetID.toString("hex")) {
      /* istanbul ignore next */
      throw new FeeAssetError(
        "Error - UTXOSet.buildExportTx: " + `feeAssetID must match cfltAssetID`
      )
    }
    ...
    if (cfltAssetID.toString("hex") === feeAssetID.toString("hex")) {
      aad.addAssetAmount(cfltAssetID, amount, fee)
    } else {
      aad.addAssetAmount(cfltAssetID, amount, zero)
      if (this._feeCheck(fee, feeAssetID)) {
        aad.addAssetAmount(feeAssetID, zero, fee)
      }
    }
   ...
   * @param startTime The Unix time when the validator starts validating the Primary Network.
   * @param endTime The Unix time when the validator stops validating the Primary Network (and staked CFLT is returned).
   * @param stakeAmount A {@link https://github.com/indutny/bn.js/|BN} for the amount of stake to be delegated in nCFLT.
  ...
  buildAddDelegatorTx = (
    ...
    if (cfltAssetID.toString("hex") === feeAssetID.toString("hex")) {
      aad.addAssetAmount(cfltAssetID, stakeAmount, fee)
    } else {
      aad.addAssetAmount(cfltAssetID, stakeAmount, zero)
      if (this._feeCheck(fee, feeAssetID)) {
        aad.addAssetAmount(feeAssetID, zero, fee)
      }
    }
