...
import { Defaults, PrimaryAssetAlias, ONECFLT } from "../../utils/constants"
...
export class AVMAPI extends JRPCAPI {
  ...
  getCFLTAssetID = async (refresh: boolean = false): Promise<Buffer> => {
    if (typeof this.CFLTAssetID === "undefined" || refresh) {
      const asset: GetCFLTAssetIDParams = await this.getAssetDescription(
        PrimaryAssetAlias
      )
      this.CFLTAssetID = asset.assetID
    }
   ...
   *
   * @param username The user paying the transaction fee (in $CFLT) for asset creation
   * @param password The password for the user paying the transaction fee (in $CFLT) for asset creation
   ...
   * [
   *   {
   *     "address": "X-cflt1kj06lhgx84h39snsljcey3tpc046ze68mek3g5",
   *     "amount": 10000
   *   },
   *   {
   *     "address": "X-cflt1am4w6hfrvmh3akduzkjthrtgtqafalce6an8cr",
   *     "amount": 50000
   *   }
   * ]
   * ```
   ...
   *    {
   *      "minters":[
   *        "X-cflt1am4w6hfrvmh3akduzkjthrtgtqafalce6an8cr"
   *      ],
   *      "threshold": 1
   *     },
   *     {
   *      "minters": [
   *        "X-cflt1am4w6hfrvmh3akduzkjthrtgtqafalce6an8cr",
   *        "X-cflt1kj06lhgx84h39snsljcey3tpc046ze68mek3g5",
   *        "X-cflt1yell3e4nln0m39cfpdhgqprsd87jkh4qnakklx"
   *      ],
   *      "threshold": 2
   *     }
   * ]
  ...
  /**
   * Send ANT (Coinflect Native Token) assets including CFLT from the X-Chain to an account on the P-Chain or C-Chain.
   *
  ...
  /**
   * Send ANT (Coinflect Native Token) assets including CFLT from an account on the P-Chain or C-Chain to an address on the X-Chain. This transaction
  ...
  buildBaseTx = async (
    ...
    const feeAssetID: Buffer = await this.getCFLTAssetID()
   ...
   * @param locktime Optional. The locktime field created in the resulting outputs
   * @param threshold Optional. The number of signatures required to spend the funds in the resultant UTXO
   * @param assetID Optional. The assetID of the asset to send. Defaults to CFLT assetID.
   * Regardless of the asset which you"re exporting, all fees are paid in CFLT.
   *
  ...
  buildExportTx = async (
    ...
    if (typeof assetID === "undefined") {
      assetID = bintools.cb58Encode(cfltAssetID)
    }
   ...
   * @param denomination Number for the denomination which is 10^D. D must be >= 0 and <= 32. Ex: $1 CFLT = 10^9 $nCFLT
   ...
   *      {
   *          "minters":[
   *              "X-cflt1ghstjukrtw8935lryqtnh643xe9a94u3tc75c7"
   *          ],
   *          "threshold": 1
   *      },
   *      {
   *          "minters": [
   *              "X-cflt1yell3e4nln0m39cfpdhgqprsd87jkh4qnakklx",
   *              "X-cflt1k4nr26c80jaquzm9369j5a4shmwcjn0vmemcjz",
   *              "X-cflt1ztkzsrjnkn0cek5ryvhqswdtcg23nhge3nnr5e"
   *          ],
   *          "threshold": 2
   *      }
   * ]
  ...
  /**
   * This class should not be instantiated directly. Instead use the [[Coinflect.addAP`${I}`]] method.
   *
