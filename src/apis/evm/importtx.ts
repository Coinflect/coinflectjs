...
export class ImportTx extends EVMBaseTx {
  ...
  private validateOuts(fee: BN): void {
    ...
    // subtract all outgoing CFLT
    this.outs.forEach((evmOutput: EVMOutput): void => {
      if (cfltAssetID === bintools.cb58Encode(evmOutput.getAssetID())) {
        feeDiff.isub(evmOutput.getAmount())
      }
    })
    if (feeDiff.lt(fee)) {
      const errorMessage: string = `Error - ${fee} nCFLT required for fee and only ${feeDiff} nCFLT provided`
