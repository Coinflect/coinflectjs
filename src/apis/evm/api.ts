...
export class EVMAPI extends JRPCAPI {
  ...
  /**
   * Send ANT (Coinflect Native Token) assets including CFLT from the C-Chain to an account on the X-Chain.
   *
   ...
   * @param to The account on the X-Chain to send the CFLT to.
  ...
  /**
   * Send CFLT from the C-Chain to an account on the X-Chain.
   *
   * After calling this method, you must call the X-Chain’s importCFLT method to complete the transfer.
   *
   * @param username The Keystore user that controls the X-Chain account specified in `to`
   * @param password The password of the Keystore user
   * @param to The account on the X-Chain to send the CFLT to.
  ...
  /**
   * Send ANT (Coinflect Native Token) assets including CFLT from an account on the X-Chain to an address on the C-Chain. This transaction
  ...
  /**
   * Send CFLT from an account on the X-Chain to an address on the C-Chain. This transaction
   * must be signed with the key of the account that the CFLT is sent from and which pays
   * the transaction fee.
   *
   * @param username The Keystore user that controls the account specified in `to`
   * @param password The password of the Keystore user
   * @param to The address of the account the CFLT is sent to. This must be the same as the to
  ...
  issueTx = async (tx: string | Buffer | Tx): Promise<string> => {
    ...
    if (typeof tx === "string") {
      ...
      throw new TransactionError(
        "Error - cflt.issueTx: provided tx is not expected type of string, Buffer, or Tx"
      )
    }
  ...
  buildExportTx = async (
    ...
    const assetDescription: any = await this.getAssetDescription("CFLT")
    ...
    if (bintools.cb58Encode(assetDescription.assetID) === assetID) {
      ...
      // if asset id isn't CFLT asset id then create 2 inputs
      // first input will be CFLT and will be for the amount of the fee
      // second input will be the ANT
      const evmCFLTInput: EVMInput = new EVMInput(
        fromAddressHex,
        fee,
        assetDescription.assetID,
        nonce
      )
      evmCFLTInput.addSignatureIdx(0, bintools.stringToAddress(fromAddressBech))
      evmInputs.push(evmCFLTInput)
   ...
   * @param baseURL Defaults to the string "/ext/bc/C/cflt" as the path to blockchain's baseURL
   * @param blockchainID The Blockchain's ID. Defaults to an empty string: ""
   */
  constructor(
    core: CoinflectCore,
    baseURL: string = "/ext/bc/C/cflt",
    blockchainID: string = ""
  ) {
    super(core, baseURL)
    this.blockchainID = blockchainID
