...
/**
 * CoinflectCore is middleware for interacting with Coinflect node RPC APIs.
 *
 * Example usage:
 * ```js
 * let coinflect = new CoinflectCore("127.0.0.1", 9650, "https")
 * ```
 *
 *
 */
export default class CoinflectCore {
  ...
  /**
   * Sets the address and port of the main Coinflect Client.
   *
   * @param host The hostname to resolve to reach the Coinflect Client RPC APIs.
   * @param port The port to resolve to reach the Coinflect Client RPC APIs.
   * @param protocol The protocol string to use before a "://" in a request,
   * ex: "http", "https", etc. Defaults to http
   * @param baseEndpoint the base endpoint to reach the Coinflect Client RPC APIs,
   * ex: "/rpc". Defaults to "/"
  ...
  setAddress = (
    ...
    if (!protocols.includes(protocol)) {
      ...
      throw new ProtocolError(
        "Error - CoinflectCore.setAddress: Invalid protocol"
      )
    }
  ...
  /**
   * Returns the host for the Coinflect node.
   */
  getHost = (): string => this.host

  /**
   * Returns the IP for the Coinflect node.
   */
  getIP = (): string => this.host

  /**
   * Returns the port for the Coinflect node.
   */
  getPort = (): number => this.port

  /**
   * Returns the base endpoint for the Coinflect node.
   */
  getBaseEndpoint = (): string => this.baseEndpoint

  /**
   * Returns the URL of the Coinflect node (ip + port)
   */
   ...
   * In TypeScript:
   * ```js
   * coinflect.addAPI<MyVMClass>("mychain", MyVMClass, "/ext/bc/mychain")
   * ```
   *
   * In Javascript:
   * ```js
   * coinflect.addAPI("mychain", MyVMClass, "/ext/bc/mychain")
   * ```
   *
  ...
  /**
   * Creates a new Coinflect instance. Sets the address and port of the main Coinflect Client.
   *
   * @param host The hostname to resolve to reach the Coinflect Client APIs
   * @param port The port to resolve to reach the Coinflect Client APIs
