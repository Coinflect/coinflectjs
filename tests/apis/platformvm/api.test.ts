...
import { ONECFLT } from "../../../src/utils/constants"
import {
  Serializable,
  Serialization,
  SerializedEncoding,
...
describe("PlatformVMAPI", (): void => {
  ...
  const username: string = "Coinflect"
  ...
  beforeAll((): void => {
    api = new PlatformVMAPI(coinflect, "/ext/bc/P")
  ...
  test("getCreateSubnetTxFee", async (): Promise<void> => {
    let pchain: PlatformVMAPI = new PlatformVMAPI(coinflect, "/ext/bc/P")
  ...
  test("refreshBlockchainID", async (): Promise<void> => {
    ...
    let testAPI: PlatformVMAPI = new PlatformVMAPI(coinflect, "/ext/bc/P")
  ...
  test("exportCFLT", async (): Promise<void> => {
    ...
    const result: Promise<string | ErrorResponseObject> = api.exportCFLT(
      username,
      password,
      amount,
      to
  ...
  test("importCFLT", async (): Promise<void> => {
    const to: string = "abcdef"
    const username: string = "Robert"
    const password = "Paulson"
    const txID = "valid"
    const result: Promise<string | ErrorResponseObject> = api.importCFLT(
      username,
      password,
      to,
      blockchainID
  ...
  describe("Transactions", (): void => {
    ...
    beforeEach(async (): Promise<void> => {
      platformvm = new PlatformVMAPI(coinflect, "/ext/bc/P")
      const result: Promise<Buffer> = platformvm.getCFLTAssetID()
      const payload: object = {
        result: {
          name,
          symbol,
      ...
      keymgr2 = new KeyChain(coinflect.getHRP(), alias)
      keymgr3 = new KeyChain(coinflect.getHRP(), alias)
      addrs1 = []
      addrs2 = []
      addrs3 = []
      utxos = []
      ...
      const amount: BN = ONECFLT.mul(new BN(amnt))
    ...
    test("signTx", async (): Promise<void> => {
      const assetID: Buffer = await platformvm.getCFLTAssetID()
    ...
    test("buildImportTx", async (): Promise<void> => {
      ...
      const txu2: UnsignedTx = set.buildImportTx(
        ...
        platformvm.getTxFee(),
        await platformvm.getCFLTAssetID(),
    ...
    test("buildExportTx", async (): Promise<void> => {
      ...
      const txu1: UnsignedTx = await platformvm.buildExportTx(
        ...
        amount,
        bintools.cb58Decode(
          Defaults.network[coinflect.getNetworkID()].X["blockchainID"]
        ),
        addrbuff3.map((a) =>
          serializer.bufferToType(a, type, coinflect.getHRP(), "P")
        ),
        addrs1,
        addrs2,
        new UTF8Payload("hello world"),
    ...
    */
    ...
    test("buildAddValidatorTx sort StakeableLockOuts 1", async (): Promise<void> => {
      // two UTXO. The 1st has a lesser stakeablelocktime and a greater amount of CFLT. The 2nd has a greater stakeablelocktime and a lesser amount of CFLT.
      ...
      const assetID = await platformvm.getCFLTAssetID()
      const assetID2 = await platformvm.getCFLTAssetID()
    ...
    test("buildAddValidatorTx sort StakeableLockOuts 2", async (): Promise<void> => {
      ...
      const outputidx1: number = 0
      const assetID: Buffer = await platformvm.getCFLTAssetID()
      const assetID2: Buffer = await platformvm.getCFLTAssetID()
      const utxo1: UTXO = new UTXO(
        codecID,
        txid,
        outputidx0,
    ...
    test("buildAddValidatorTx sort StakeableLockOuts 3", async (): Promise<void> => {
      ...
      // The 2nd has a lesser stakeablelocktime and a greater amount of CFLT.
      // The 3rd has a greater stakeablelocktime and a lesser amount of CFLT.
      //
    ...
    test("buildAddValidatorTx 2", async (): Promise<void> => {
      ...
      const addrbuff2 = addrs2.map((a) => platformvm.parseAddress(a))
      const addrbuff3 = addrs3.map((a) => platformvm.parseAddress(a))
      const amount: BN = ONECFLT.mul(new BN(25))

      const locktime: BN = new BN(54321)
      const threshold: number = 2

      platformvm.setMinStake(ONECFLT.mul(new BN(25)), ONECFLT.mul(new BN(25)))
    ...
    test("buildAddValidatorTx 3", async (): Promise<void> => {
      ...
      const addrbuff2 = addrs2.map((a) => platformvm.parseAddress(a))
      const addrbuff3 = addrs3.map((a) => platformvm.parseAddress(a))
      const amount: BN = ONECFLT.mul(new BN(3))

      const locktime: BN = new BN(54321)
      const threshold: number = 2

      platformvm.setMinStake(ONECFLT.mul(new BN(3)), ONECFLT.mul(new BN(3)))

      //2 utxos; one lockedstakeable; other unlocked; both utxos have 2 cflt; stake 3 CFLT

      const dummySet: UTXOSet = new UTXOSet()

      const lockedBaseOut: SECPTransferOutput = new SECPTransferOutput(
        ONECFLT.mul(new BN(2)),
        addrbuff1,
        locktime,
        1
      )
