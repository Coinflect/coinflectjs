...
import { CENTICFLT } from "src/utils"
import { MILLICFLT } from "src/utils"
...
describe("AVMAPI", (): void => {
  ...
  beforeAll((): void => {
    api = new AVMAPI(coinflect, "/ext/bc/X", blockchainID)
  ...
  test("refreshBlockchainID", async (): Promise<void> => {
    ...
    const testAPI: AVMAPI = new AVMAPI(coinflect, "/ext/bc/avm", n3bcID)
  ...
  test("createFixedCapAsset", async (): Promise<void> => {
    const kp: KeyPair = new KeyPair(coinflect.getHRP(), alias)
    kp.importKey(
      Buffer.from(
  ...
  describe("Transactions", (): void => {
    ...
    beforeEach(async (): Promise<void> => {
      avm = new AVMAPI(coinflect, "/ext/bc/X", blockchainID)

      const result: Promise<Buffer> = avm.getCFLTAssetID(true)
      const payload: object = {
        result: {
          name,
          symbol,
    ...
    test("buildCreateNFTMintTx", async (): Promise<void> => {
      ...
      const payload: Buffer = Buffer.from("Coinflect")
    ...
    test("buildExportTx", async (): Promise<void> => {
      ...
      const txu1: UnsignedTx = await avm.buildExportTx(
        ...
        addrbuff3.map((a: Buffer): any =>
          serialization.bufferToType(a, type, coinflect.getHRP(), "P")
        ),
        addrs1,
        addrs2,
        new UTF8Payload("hello world"),
