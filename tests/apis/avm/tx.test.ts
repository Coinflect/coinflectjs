...
import { Coinflect } from "../../../src/index"
...
describe("Transactions", (): void => {
  ...
  beforeAll(async (): Promise<void> => {
    ...
    api = new AVMAPI(coinflect, "/ext/bc/avm", bID)

    const result: Promise<Buffer> = api.getCFLTAssetID()
    ...
    cfltAssetID = await result
  })

  beforeEach((): void => {
    set = new UTXOSet()
    keymgr1 = new KeyChain(coinflect.getHRP(), alias)
    ...
    amount = ONECFLT.mul(new BN(amnt))
