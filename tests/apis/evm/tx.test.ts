...
describe("EVM Transactions", () => {
  describe("ImportTx", () => {
    test("Multiple CFLT EVMOutput fail", (): void => {
      const outputidx: Buffer = Buffer.from("")
      const input: SECPTransferInput = new SECPTransferInput(ONECFLT)
      const xferin: TransferableInput = new TransferableInput(
        bintools.cb58Decode(txID),
        outputidx,
        bintools.cb58Decode(cfltAssetID),
        input
      )
      importedIns.push(xferin)
      // Creating 2 outputs with the same address and CFLT assetID is invalid
      ...
      evmOutput = new EVMOutput(cHexAddress1, ONECFLT, cfltAssetID)
      evmOutputs.push(evmOutput)
    ...
    test("Multiple CFLT EVMOutput success", (): void => {
      const outputidx: Buffer = Buffer.from("")
      const input: SECPTransferInput = new SECPTransferInput(ONECFLT)
      const xferin: TransferableInput = new TransferableInput(
        bintools.cb58Decode(txID),
    ...
    test("Multiple ANT EVMOutput fail", (): void => {
      ...
      evmOutputs.push(evmOutput)
      evmOutput = new EVMOutput(cHexAddress1, ONECFLT, antAssetID)
      evmOutputs.push(evmOutput)
      expect((): void => {
        new ImportTx(
          networkID,
    ...
    test("Multiple ANT EVMOutput success", (): void => {
      ...
      evmOutputs.push(evmOutput)
      evmOutput = new EVMOutput(cHexAddress2, ONECFLT, antAssetID)
      evmOutputs.push(evmOutput)
    ...
    test("Single ANT EVMOutput fail", (): void => {
      ...
      // If the output is a non-cflt assetID then don't subtract a fee
      ...
      expect((): void => {
        ...
        "Error - 25000000000 nCFLT required for fee and only 0 nCFLT provided"
      )
    })
    ...
    test("Single CFLT EVMOutput fail", (): void => {
      const outputidx: Buffer = Buffer.from("")
      const input: SECPTransferInput = new SECPTransferInput(new BN(507))
      const xferin: TransferableInput = new TransferableInput(
        bintools.cb58Decode(txID),
      ...
      expect((): void => {
        ...
        "Error - 25000000000 nCFLT required for fee and only 507 nCFLT provided"
      )
    })

    test("Single CFLT EVMOutput success", (): void => {
      const outputidx: Buffer = Buffer.from("")
      const input: SECPTransferInput = new SECPTransferInput(ONECFLT)
      const xferin: TransferableInput = new TransferableInput(
        bintools.cb58Decode(txID),
