...
describe("EVMAPI", (): void => {
  ...
  beforeAll((): void => {
    api = new EVMAPI(coinflect, "/ext/bc/C/cflt", blockchainID)
  })
  ...
  test("exportCFLT", async (): Promise<void> => {
    ...
    let result: Promise<string> = api.exportCFLT(username, password, to, amount)
    let payload: object = {
      result: {
        txID: txID
      }
  ...
  test("importCFLT", async (): Promise<void> => {
    ...
    let password: string = "Paulson"
    let txID: string = "valid"
    let result: Promise<string> = api.importCFLT(
      username,
      password,
      to,
      blockchainID
  ...
  test("refreshBlockchainID", async (): Promise<void> => {
    ...
    const testAPI: EVMAPI = new EVMAPI(coinflect, "/ext/bc/C/cflt", n5bcID)
