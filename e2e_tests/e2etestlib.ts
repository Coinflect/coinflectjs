...
export const getCoinflect = (): Coinflect => {
  if (typeof process.env.COINFLECTCHAIN_IP === "undefined") {
    throw "Undefined environment variable: COINFLECTCHAIN_IP"
  }
  if (typeof process.env.COINFLECTCHAIN_PORT === "undefined") {
    throw "Undefined environment variable: COINFLECTCHAIN_PORT"
  }
  const coinflect: Coinflect = new Coinflect(
    process.env.COINFLECTCHAIN_IP,
    parseInt(process.env.COINFLECTCHAIN_PORT)
  )
  return coinflect
}
