...
describe("PChain", (): void => {
  ...
  const coinflect = getCoinflect()
  const pchain = coinflect.PChain()
  const keystore = new KeystoreAPI(coinflect)
  ...
  const user: string = "coinflectJspChainUser"
  ...
  const coinflectBlockChainID: string =
  ...
  const tests_spec: any = [
    ...
    [
      "getBlockchainStatus",
      () => pchain.getBlockchainStatus(coinflectBlockChainID),
      (x) => x,
      Matcher.toBe,
      () => "Unknown"
    ],
    ...
    // [
    //   "exportCFLT",
    //   () => pchain.exportCFLT(user, passwd, new BN(10), xChainAddr),
    //   (x) => x,
    //   Matcher.toThrow,
    //   () =>
    ...
    [
      "importCFLT",
      () => pchain.importCFLT(user, passwd, addrB.value, "X"),
      (x) => x,
      Matcher.toThrow,
