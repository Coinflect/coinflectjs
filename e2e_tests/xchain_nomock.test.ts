...
describe("XChain", (): void => {
  ...
  const xchain = coinflect.XChain()
  const keystore = new KeystoreAPI(coinflect)

  const user: string = "coinflectJsXChainUser"
  const passwd: string = "coinflectJsP1ssw4rd"
  ...
  const tests_spec: any = [
    ...
    [
      "getBalance",
      () => xchain.getBalance(whaleAddr, "CFLT"),
      (x) => x.balance,
      Matcher.toBe,
      () => "300000000000000000"
    ],
    ...
    [
      ...
      () =>
        xchain.sendMultiple(
          ...
          passwd,
          [
            { assetID: "CFLT", amount: 10, to: addrB.value },
            { assetID: "CFLT", amount: 20, to: addrC.value }
          ],
          [whaleAddr],
          whaleAddr,
          memo
