...
import { InitialStates } from "coinflect/dist/apis/avm"
import {
  PlatformVMAPI,
  KeyChain,
  SECPTransferOutput,
...
const fee: BN = ONECFLT
const threshold: number = 1
const locktime: BN = new BN(0)
const cfltUTXOKeychain: Buffer[] = [pAddresses[0], pAddresses[1]]
const cfltUTXOKeychainStrings: string[] = [
  pAddressStrings[0],
  pAddressStrings[1]
]
...
const main = async (): Promise<any> => {
  ...
  const memo: Buffer = Buffer.from(
    "Manually create a CreateChainTx which creates a 1-of-2 CFLT utxo and instantiates a VM into a blockchain by correctly signing the 2-of-3 SubnetAuth"
  )
